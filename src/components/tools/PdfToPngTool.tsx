    ...
    <div className="space-y-5">
      {!hasFile ? (
        ...
        <>
          {loading && (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm text-muted-foreground">
                <span className="animate-pulse-soft">Converting pages to PNG...</span>
                <span>{progress}%</span>
              </div>
          ...
          {images.length > 0 && (
            <>
              <Button onClick={downloadAll} className="w-full h-11 rounded-xl gap-2">
                <Download className="h-4 w-4" /> Download All {images.length} Pages as PNG
              </Button>
              <div className="grid gap-4 sm:grid-cols-2">
                {images.map((img, i) => (
                  <div key={i} className="rounded-2xl border border-border p-3 shadow-soft">
                    ...
                    <Button onClick={() => download(img, i)} variant="outline" size="sm" className="w-full mt-2 rounded-xl">
                      Download Page {i + 1}
                    </Button>
                  </div>
                ))}
          ...
          <Button variant="ghost" size="sm" className="w-full text-muted-foreground" onClick={() => { setHasFile(false); setImages([]); }}>
